...
for i = 3%:length(fine_lp_data)
    ...
    for j = 1%size(fine_lp_data{1}{1},1)
        ...
        for m = 1:20%nIter
            ...
            for n = 1:stepSize:length(l)%nFrames
                %idxStart = 1 + (n-1) * (lenFrames - lenOverlap);
                ...
                % line to modify, for creating RLS algorithm
                % calculate k
                LP = fft(lp);
                lk = (LP * LgJ) ./ (lambda + LgJ' * LP * LgJ);
                LP = (LP - lk * LgJ' * LP)./lambda;
                lp = ifft(LP);

                LGh = LGh + lk.*LgJ.*cLENH_FINE;
                ...
                % calculate k
                RP = fft(rp);
                rk = (RP * RgJ) ./ (lambda + RgJ' * RP * RgJ);
                RP = (RP - rk * RgJ' * RP)./lambda;
                rp = ifft(RP);

                RGh = RGh + rk.*RgJ.*cRENH_FINE;
